...
A python library that uses browser automation to login & scrape messages from linkedin
...
```sh
pip install linkedin-message-scraper
```

### Import

```javascript
from linkedin-message-scraper import *
```

### Login

To scrape chat messages from linkedin first we will login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To scrape chat messages with particular user on linkedin we use ``get_messages`` function.
It requires **profile_link** as input parameter

```javascript
response=linkedin.get_messages(profile_link="profile_link_here")
